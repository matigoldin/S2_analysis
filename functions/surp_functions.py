...
def IsSig(Surp,th,wb,nconsecabove,surpmin,nbinsabove):
    ...
    for w in np.arange(25):
        ...
        if Sig[w][0]==1:
            nhightot[w][0] = sum(sumoverthresh[:,0])*Sig[w][0]
            #nhightot[w][0] = sum(two_consec[:,0])*sum(consec[:,0]*two_consec[:,0])
        if Sig[w][1]==1:
            nhightot[w][1] = sum(sumoverthresh[:,1])*Sig[w][1]
    ...
    # Selecting PW as blank whisker if there is no responses
    if m0==0: 
        PW[0]=wb
    if m1==0: 
        PW[1]=wb
...
#---------------------------------------------------------------------------------------
def GetLatency(surp,detected, surpmax, maxbin, surpmin, th):

    # surp: surprise values   
    # detected: detection data
    # surpmax: the maximum bin to compute surprise
    # maxbin: number of different binsizes the surprise was computed
    # surpmin: minimum latency in ms, from where we compute
    # th: thresholds from the blank surprise

    siglist = detected.Sig_sizes
    sig = detected.Sig
        ...
        for d in [0,1]:
            if sig[w][d] == 1: # if whisker and direction is significant, then compute latency
                ...
                for binsize in np.arange(maxbin)+1: # for each binsize we compute the minimum latency
                    ...
                    maxsurpbin = np.where( surp[w][binsize][d]==max(surp[w][binsize][d]) )[0][0]

                    #we decrease -1 for the way np.arange creates arrays and search backwards to get minbin
                    for b in np.arange(surpmin-1,surpmax)[::-1]:  
                        #compute the big threshold surprise, while it is at least preceding the max by 5ms
                        if surp[w][binsize][d][b]-th[binsize-1]>0 and b<minbin and maxsurpbin-b<=5 and maxsurpbin-b>=0:
                            minbin=b
                    # now we search backwards to get minbin50
                    for b in np.arange(surpmin-1,surpmax)[::-1]:  
                        #compute the small threshold surprise, , while it is at least preceding the bigthsurp by 5ms
                        if surp[w][binsize][d][b]-th[binsize-1]*0.5>0 and b<minbin50 and minbin-b<=5 and minbin-b>=0:
                            minbin50=b

                    # if small threshold surprise does not preceed big threshold (not found), we keep big threshold
                    if minbin-minbin50 > 5:
                        minbin50 = minbin

                    # if before the bin selected it does not return to slightly below chance level, keep the big
                    if min(surp[w][binsize][d][0:minbin50])>th[binsize-1]:
                        minbin50 = minbin
                    
                    # Here we compare with global min for different binsizes    
